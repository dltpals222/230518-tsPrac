...
function spacingLetter(text:string, nounData:string[], spacing:string) : string{
  let result : string= "";
  let findText = (element:string, indexText:string) : string => {
  ...
  nounData.map((element:string,i:number) : void => {
    if(i === 0){
...
const spacingResult :string = spacingLetter(dummyText,reFineNounData,spacing)
